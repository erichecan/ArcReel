...
4. **资产身份确认（必经，不可跳过；这一步只定名字，不出图）**：
   - `next_action.type == "confirm_ad_asset_plan"` 时：分析 `brief`/卖点里会在多个视频单元/分镜里反复出现的角色、场景、道具，逐条向用户确认是否需要登记；用户确认的条目经 `patch_project` 写入 `characters`/`scenes`/`props` 即可调用 `mcp__arcreel__confirm_ad_asset_plan({})`，不必这一步就 dispatch `generate-assets` 出图——脚本还没写，出图目标不明确，等步骤 5 脚本把这些名字写成 `@[名称]` 引用之后再出图更准。真的不需要任何资产时传 `{"no_additional_assets": true}`；工具在没有登记任何资产且未传该参数时会拒绝，需要退回去补登记或补传参数。这一步不确认，计划不会推进到步骤 5。
   - `next_action.type == "generate_asset_sheets"` 时（脚本生成后，工作流检测到已登记资产仍缺 sheet 会自动切到这个动作）：对每个类型取 `artifacts.asset_sheets[type].missing_ids` 与 `requested_ids` 的交集作为该类型的 `names`，调用 `mcp__arcreel__generate_assets({"type": type, "names": [该类型 names]})`。商品 sheet 在商品资产页生成。
