...
- **资产身份确认（必经）**：剧本生成前工作流会停在 `confirm_ad_asset_plan` 这一步，不能跳过，但这一步只登记「有哪些资产、叫什么名字」，不出图。卖点确认后，先分析 `brief`/卖点里会反复出现的角色（如「老板」「主厨」）、场景（如「后厨」「前厅」）、道具（如某个招牌菜或关键实物），逐条列给用户确认是否需要登记为资产——尤其是「同一个人物/场景/物件会在多个视频单元里反复出现」的内容（探店、菜谱类最典型），脚本里缺这些名字会导致各视频单元各自臆造实体、互相对不上。用户确认需要的资产登记进 `project.json`（`patch_project` 写名字与描述）后即可调用 `mcp__arcreel__confirm_ad_asset_plan({})`，不必先出图——资产图留到脚本生成之后，工作流会在脚本就绪、发现资产缺 sheet 时自动把 `next_action` 切到 `generate_asset_sheets`，届时再 dispatch `generate-assets` 子智能体出图（脚本已经把这些名字写进 `@[名称]` 引用，出图目标更明确，也避免图先出好、脚本却没用上或改了名字要重出）。确实是纯氛围空镜、没有反复出现实体的极简短片，才能对用户说明后调用 `mcp__arcreel__confirm_ad_asset_plan({"no_additional_assets": true})` 跳过；否则调用该工具时不传该参数（默认 false），工具会在没有登记任何资产时拒绝确认
- **剧本**：生成前先判断 `brief`/卖点是否指向具体菜品、菜谱这类有客观标准工序的内容——是的话先用 `WebSearch` 查证真实制作流程（尤其是否有预处理、过油炸、腌制这类容易被脚本遗漏、但直接决定最终画面是否可信的关键工序），把查到的工序要点带入生成，避免脚本工序链条不完整导致视频生成模型瞎画；其余内容不必搜索。`mcp__arcreel__generate_episode_script({"episode": 1})` 单阶段产出，八段带货框架按 `target_duration` 选档配比；分镜图生视频路径向用户呈现分镜列表与口播文案，参考生视频路径呈现视频单元列表与引用语法正文，按需经 `patch_episode_script` 调整（顺序调整引导用户到 WebUI 剧本页）
