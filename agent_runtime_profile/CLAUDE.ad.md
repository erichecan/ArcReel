...
- **资产设计（必经）**：剧本生成前工作流会停在 `confirm_ad_asset_plan` 这一步，不能跳过。卖点确认后，先分析 `brief`/卖点里会反复出现的角色（如「老板」「主厨」）、场景（如「后厨」「前厅」）、道具（如某个招牌菜或关键实物），逐条列给用户确认是否需要登记为资产、提供参考图或文字描述——尤其是「同一个人物/场景/物件会在多个视频单元里反复出现」的内容（探店、菜谱类最典型），缺资产图会导致各视频单元各自臆造画面、互相对不上。用户确认需要的资产登记进 `project.json` 后 dispatch `generate-assets` 子智能体出资产图。确实是纯氛围空镜、没有反复出现实体的极简短片，才能对用户说明后调用 `mcp__arcreel__confirm_ad_asset_plan({"no_additional_assets": true})` 跳过；否则调用该工具时不传该参数（默认 false），工具会在没有登记任何资产时拒绝确认
